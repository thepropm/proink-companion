...
function Home({ onOpen }: { onOpen: (screen: Screen) => void }) { return <main className="eink-main"><button className="continue-card" onClick={() => onOpen("reader")}><span className="continue-kicker">NOW READING</span><div className="continue-body"><span className="home-book-cover"><small>PROINK<br />READS</small><b>The<br />Art of<br />Reading</b><em>M. Ito</em></span><span className="continue-details"><strong>The Art of Reading</strong><span className="book-meta">M. Ito <i /> 42% complete</span><span className="book-progress"><b style={{ width: "42%" }} /></span><span className="book-stats"><span><b>6</b>Sessions</span><span><b>2h 18m</b>Read time</span><span><b>5.4</b>Pages/min</span><span><b>23m</b>Avg session</span><span><b>Sep 02</b>Started</span><span><b>42%</b>Progress</span></span></span></div><span className="reading-rhythm"><span><b>4 days</b>Streak</span><span><b>12</b>Books read</span><span><b>Evening</b>Best time</span><span><b>Sunday</b>Most read</span></span><em>Continue reading</em></button><div className="app-grid">{APPS.map(([label, Icon]) => <button key={label} className="app-tile" onClick={() => onOpen(label === "Library" ? "library" : label === "Settings" ? "settings" : "home")}><Icon size={28} weight="regular" /><span>{label}</span></button>)}</div></main>; }
