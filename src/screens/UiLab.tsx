...
function FrontLight({ onBack }: { onBack: () => void }) {
  ...
  return <main className="eink-main eink-settings eink-frontlight"><div className="screen-title"><button onClick={onBack}>‹</button><strong>Front light</strong></div><p className="settings-intro">Light is applied only when you confirm a change.</p><button className="setting-toggle" onClick={() => setEnabled((value) => !value)}><span><strong>Front light</strong><small>{enabled ? "On" : "Off"}</small></span><i className={enabled ? "switch on" : "switch"} /></button><fieldset disabled={!enabled}><label className="setting-range"><span>Brightness <b>{brightness}%</b></span><input type="range" min="0" max="100" step="1" value={brightness} onChange={(event) => setBrightness(Number(event.target.value))} /></label><label className="setting-range"><span>Warmth <b>{warmth}%</b></span><input type="range" min="0" max="100" step="1" value={warmth} onChange={(event) => setWarmth(Number(event.target.value))} /></label></fieldset><section className="schedule-card"><button className="setting-toggle" onClick={() => setScheduled((value) => !value)}><span><strong>Schedule</strong><small>{scheduled ? "On" : "Off"}</small></span><i className={scheduled ? "switch on" : "switch"} /></button><button disabled={!scheduled} onClick={() => setStart((value) => cycleTime(value, ["7:00 PM", "8:00 PM", "9:00 PM", "10:00 PM"]))}><span>Start</span><b>{start}</b></button><button disabled={!scheduled} onClick={() => setEnd((value) => cycleTime(value, ["6:00 AM", "7:00 AM", "8:00 AM", "9:00 AM"]))}><span>End</span><b>{end}</b></button></section></main>;
}
